...
import { type RecentTransactionItem, useCashflowStore } from "@/store/cashflow-store";
  ...
  const recentTransactions = useMemo(
    () =>
      [...transactions]
        ...
        .map(formatRecentTransaction),
    [transactions]
  );
  const recentExpenseTransactions = useMemo(
    () => recentTransactions.filter((transaction) => transaction.type === "expense"),
    [recentTransactions]
  );
    ...
    <div className="space-y-8">
      <div className="rounded-xl border bg-card p-5 md:p-6">
        ...
        <p className="mt-2 max-w-3xl whitespace-nowrap text-sm leading-6 text-muted-foreground">
          실제 지출과 수입을 함께 확인하고, 최근 발생한 내역을 빠르게 관리하는 화면입니다.
        </p>
      </div>
      ...
      <div className="grid gap-8 xl:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
        <div className="space-y-8">
          ...
          <RecentExpenseList expenses={recentExpenseTransactions} onDelete={handleDeleteTransaction} />
        </div>
